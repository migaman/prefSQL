...
namespace Utility
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            String str1 = "SELECT cars_superlarge.id , cars_superlarge.price, cars_superlarge.mileage, cars_superlarge.horsepower, cars_superlarge.enginesize, cars_superlarge.registration, cars_superlarge.consumption, cars_superlarge.doors, CASE WHEN colors.name = 'rot' THEN 0 WHEN colors.name = 'blau' THEN 0 WHEN colors.name = 'grau' THEN 200 ELSE 100 END, CASE WHEN colors.name = 'rot' THEN '' WHEN colors.name = 'blau' THEN '' WHEN colors.name = 'grau' THEN ''ELSE colors.name END, CASE WHEN fuels.name = 'Benzin' THEN 0 WHEN fuels.name = 'Diesel' THEN 200 ELSE 100 END, CASE WHEN fuels.name = 'Benzin' THEN '' WHEN fuels.name = 'Diesel' THEN ''ELSE fuels.name END, CASE WHEN bodies.name = 'Kleinwagen' THEN 0 WHEN bodies.name = 'Bus' THEN 100 WHEN bodies.name = 'Kombi' THEN 200 WHEN bodies.name = 'Roller' THEN 300 WHEN bodies.name = 'Pick-Up' THEN 500 ELSE 400 END, CASE WHEN bodies.name = 'Kleinwagen' THEN '' WHEN bodies.name = 'Bus' THEN '' WHEN bodies.name = 'Kombi' THEN '' WHEN bodies.name = 'Roller' THEN '' WHEN bodies.name = 'Pick-Up' THEN ''ELSE bodies.name END, CASE WHEN cars_superlarge.title = 'MERCEDES-BENZ SL 600' THEN 0 ELSE 100 END, CASE WHEN cars_superlarge.title = 'MERCEDES-BENZ SL 600' THEN ''ELSE cars_superlarge.title END, CASE WHEN makes.name = 'ASTON MARTIN' THEN 0 WHEN makes.name = 'VW' THEN 100 WHEN makes.name = 'Audi' THEN 100 WHEN makes.name = 'FERRARI' THEN 300 ELSE 200 END, CASE WHEN makes.name = 'ASTON MARTIN' THEN '' WHEN makes.name = 'VW' THEN '' WHEN makes.name = 'Audi' THEN '' WHEN makes.name = 'FERRARI' THEN ''ELSE makes.name END, CASE WHEN conditions.name = 'Neu' THEN 0 ELSE 100 END, CASE WHEN conditions.name = 'Neu' THEN ''ELSE conditions.name END FROM cars_superlarge LEFT OUTER JOIN colors ON cars_superlarge.color_id = colors.ID LEFT OUTER JOIN fuels ON cars_superlarge.fuel_id = fuels.ID LEFT OUTER JOIN bodies ON cars_superlarge.body_id = bodies.ID LEFT OUTER JOIN makes ON cars_superlarge.make_id = makes.ID LEFT OUTER JOIN conditions ON cars_superlarge.condition_id = conditions.ID ORDER BY price ASC, mileage ASC, horsepower DESC, enginesize DESC, registration DESC, consumption ASC, doors DESC, CASE WHEN colors.name = 'rot' THEN 0 WHEN colors.name = 'blau' THEN 0 WHEN colors.name = 'grau' THEN 200 ELSE 100 END ASC, CASE WHEN fuels.name = 'Benzin' THEN 0 WHEN fuels.name = 'Diesel' THEN 200 ELSE 100 END ASC, CASE WHEN bodies.name = 'Kleinwagen' THEN 0 WHEN bodies.name = 'Bus' THEN 100 WHEN bodies.name = 'Kombi' THEN 200 WHEN bodies.name = 'Roller' THEN 300 WHEN bodies.name = 'Pick-Up' THEN 500 ELSE 400 END ASC, CASE WHEN cars_superlarge.title = 'MERCEDES-BENZ SL 600' THEN 0 ELSE 100 END ASC, CASE WHEN makes.name = 'ASTON MARTIN' THEN 0 WHEN makes.name = 'VW' THEN 100 WHEN makes.name = 'Audi' THEN 100 WHEN makes.name = 'FERRARI' THEN 300 ELSE 200 END ASC, CASE WHEN conditions.name = 'Neu' THEN 0 ELSE 100 END ASC";
            String str2 = ";LOW;LOW;HIGH;HIGH;HIGH;LOW;HIGH;LOW;INCOMPARABLE;LOW;INCOMPARABLE;LOW;INCOMPARABLE;LOW;INCOMPARABLE;LOW;INCOMPARABLE;LOW;INCOMPARABLE";
            String str3 = "SELECT cars_superlarge.price,cars_superlarge.mileage,cars_superlarge.horsepower,cars_superlarge.enginesize,cars_superlarge.registration,cars_superlarge.consumption,cars_superlarge.doors,colors.name,fuels.name,bodies.name,cars_superlarge.title,makes.name,conditions.name FROM cars_superlarge LEFT OUTER JOIN colors ON cars_superlarge.color_id = colors.ID LEFT OUTER JOIN fuels ON cars_superlarge.fuel_id = fuels.ID LEFT OUTER JOIN bodies ON cars_superlarge.body_id = bodies.ID LEFT OUTER JOIN makes ON cars_superlarge.make_id = makes.ID LEFT OUTER JOIN conditions ON cars_superlarge.condition_id = conditions.ID ";
            String str4 = "cars_superlarge";
        ...
        public void Run()
        {
            try
            {
                ...
                */
                ...
                String strPrefSQL = "SELECT cars.id, cars.title, cars.price, cars.mileage FROM cars " +
                ...
                "WHERE cars.price > 10000 PREFERENCE LOW cars.price AND Low cars.mileage ";
