...
namespace Utility
{
    ...
    class Performance
    {
        ...
        public void GeneratePerformanceQueries()
        {
            ...
            DataTable dt = parser.ParseAndExecutePrefSQL(Helper.ConnectionString, Helper.ProviderName, "SELECT CAST(cars.id AS BIGINT) FROM cars SKYLINE OF cars.price LOW");
            ...
            foreach(SkylineStrategy currentStrategy in listStrategy)
            {
                ...
                for (int iPreferenceIndex = 0; iPreferenceIndex < listPreferences.Count; iPreferenceIndex++)
                {
                    ...
                    //SELECT FROM
                    string strSQL = "SELECT CAST CAST(cars.id AS BIGINT) FROM ";
        ...
        private DataTable GetSQLFromPreferences(ArrayList preferences, bool cardinality)
        {
            ...
            string strPrefSQL = "SELECT CAST(cars.id AS BIGINT) FROM ";
            if (TableSize == Size.Small)
            {
